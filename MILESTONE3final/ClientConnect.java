...
public class ClientConnect {
	...
	/*
	public void SendplaceBoatCoords(int x, int y) {
		out.writeobject(new Payload());
		
	}
	*/
	public void placeBoat(String choice) {
		try {
	...
	/*if(coordinateListPlaced.size()< MAX_POINTS) {
	
		coordinateListPlaced.add(coordinates.getLocation());
		}else {
	...
	*/
	...
	public void sendGuess(String guess) {
		try {
			out.writeObject(new Payload(PayloadType.GUESS, guess));
		} catch (IOException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
	...
	void processPayload(Payload p) {
		switch(p.payloadType) {
			...
			case MESSAGE:
				...
				break;
			case CHOICE:
				System.out.println("Got COORDINATES " + p.message);
				messages.add(p.message);
				break;
			case GUESS:
				System.out.println("Ship guessed " + p.message);
				messages.add(p.message);
				break;
			case UPDATE_NAME:
